...
Nginx cache 앞단을 테스트하려면 Nginx cache service를 띄우고 redirect target을 `http://host.docker.internal:8088`로 바꾼다.

```bash
docker compose up -d nginx-cache
docker compose run --rm \
  -e REDIRECT_BASE_URL=http://host.docker.internal:8088 \
...
- `long_query_redirect`: 같은 short code를 redirect하되 긴 query string과 긴 trace header를 붙인다. 캐시는 hit이어도 request guard, access log 직렬화, 파일 쓰기 경로가 커지는지 본다.
- `invalid_create_payloads`: 2048자를 넘는 URL 생성 요청을 반복해 application validation이 code 생성과 DB 조회 전에 400으로 빠르게 끝나는지 본다. payload 자체가 body 한도를 넘으면 request size guard가 컨트롤러/Jackson parsing 전에 413으로 자른다.
...
| `HTTP_TIMEOUT` | `2s` | pathological-inputs 개별 HTTP 요청 timeout |

Management Server는 `short-url.management.create.max-request-body-bytes=8192`을 기본값으로 둔다. 이 값은 논리적으로 invalid인 긴 URL JSON은 애플리케이션 validation까지 도달하게 두고, 진짜 oversized body는 컨트롤러/Jackson 파싱 전에 `413 Payload Too Large`로 자르기 위한 경계다. 기본 `short-url.management.create.max-drain-body-bytes=0`에서는 한도를 넘는 body를 drain하지 않고 `Connection: close`로 응답해서 worker thread가 큰 body를 읽느라 오래 묶이지 않게 한다.

Redirect Server는 `short-url.redirect.request.max-uri-chars=4096`, `short-url.redirect.request.max-query-chars=2048`을 기본값으로 둔다. 이 한도를 넘는 redirect 요청은 upstream 캐시/DB 경로로 내려가지 않고 `414 URI Too Long`으로 끝난다. 이 414 응답은 keep-alive를 유지해서 긴 query 요청이 반복될 때 TCP connection churn이 병목이 되지 않게 한다.
...
- PostgreSQL primary/replica connection split
- Valkey master/replica health
- JVM memory/thread
...
- replica lag가 커지면 방금 생성한 code의 redirect가 일시적으로 miss될 수 있으므로 PostgreSQL replica 상태와 negative cache TTL을 같이 본다.
