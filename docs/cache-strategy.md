...
현재 전략은 Caffeine local cache와 Valkey/Redis remote cache를 함께 쓰는 2단 cache-aside/read-through에 가깝다. Valkey는 master-replica로 구성하며, Spring Data Redis/Lettuce는 write command를 master로 보내고 read command는 replica를 우선 사용한다.
...
PostgreSQL도 primary-replica 구성을 지원한다. `@Transactional(readOnly=true)` repository 조회는 replica로 라우팅하고, 저장, Flyway migration, read-only가 아닌 transaction은 primary로 라우팅한다.
...
DB replica read는 primary보다 늦을 수 있다. 특히 redirect miss를 negative cache하는 구조에서는 replica lag 동안 조회한 `NOT_FOUND`가 짧게 남을 수 있다. 운영에서는 replica lag alert, 짧은 negative TTL, 새로 생성된 code의 warm-up, 또는 not found primary fallback을 추가로 검토한다.
